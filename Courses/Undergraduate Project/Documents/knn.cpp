...
namespace GRT
{
    ...
    bool KNN::predict(vector<double> inputVector, UINT K)
    {
        ...
        if (classLikelihoods.size() != numClasses)
        ...
        if (classDistances.size() != numClasses)
        ...
        for (UINT i = 0; i < classDistances.size(); i++)
        {
            if (classLikelihoods[i] > 0)
                classDistances[i] /= classLikelihoods[i];
            else
                classDistances[i] = BIG_DISTANCE;
        }
