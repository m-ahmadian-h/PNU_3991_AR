...
int OnInit()
  ...
  endSymbol=StringSubstr(Symbol(),6,StringLen(Symbol()));
   ...
   //--- indicator buffers mapping
      ...
      SetIndexStyle (0,DRAW_HISTOGRAM,STYLE_SOLID,13,clrBlue);// Line style
      SetIndexLabel(0,"powerLong");
      
      SetIndexBuffer(1,Buf_1);         // Assigning an array to a buffer
      SetIndexStyle (1,DRAW_HISTOGRAM,STYLE_SOLID,13,clrRed);// Line style
      SetIndexLabel(1,"powerShort");
      
      if(IsTesting()){
         string nameD="commentShow1";
         ObjectCreate(0,nameD,OBJ_LABEL,0,0,0);
         ObjectSetInteger(0,nameD,OBJPROP_COLOR,clrSandyBrown);
                ...
                if(edi1<=0)
                  {
                   objB.setText("edit1","0");
                    ...
                    periods=60;
                    if(!IsTesting()){runFirstABbb(edi1);}
                  }
                objB.setColorClick("text6");
                objB.offColorClick("text7");
                objB.offColorClick("text8");
   ...
   for(int i=1;i<History;i++)
    {
       ...
       Buf_0[i]=powerBaseLongIMAEUR_AB[i]-powerBaseLongIMAUSD_AB[i];
       Buf_1[i]=powerBaseShortIMAEUR_AB[i]-powerBaseShortIMAUSD_AB[i];
   ...
   if(fraw)
      {
       ...
       if(ChartScaleGet()==4)
         {
            SetIndexStyle (0,DRAW_HISTOGRAM,STYLE_SOLID,7,clrBlue);// Line style
            SetIndexStyle (1,DRAW_HISTOGRAM,STYLE_SOLID,7,clrRed);// Line style
            changeWigh(1);
         }
       if(ChartScaleGet()==3)
         {
            SetIndexStyle (0,DRAW_HISTOGRAM,STYLE_SOLID,3,clrBlue);// Line style
            SetIndexStyle (1,DRAW_HISTOGRAM,STYLE_SOLID,3,clrRed);// Line style
            changeWigh(0.9);
         }
       if(ChartScaleGet()<=2)
         {
            SetIndexStyle (0,DRAW_HISTOGRAM,STYLE_SOLID,2,clrBlue);// Line style
            SetIndexStyle (1,DRAW_HISTOGRAM,STYLE_SOLID,2,clrRed);// Line style
            changeWigh(0.1);
         }
    }
    else{
       hiddenObject();
       SetIndexStyle (0,DRAW_NONE,STYLE_SOLID,1,clrBlue);// Line style
       SetIndexStyle (1,DRAW_NONE,STYLE_SOLID,1,clrRed);// Line style
    }
}  
int lasBBB=Bars;
bool creatBBB=false;
           ...
           if(lastBar!=Bars)
             {
               ...
               if(Buf_0[i+1]<Buf_1[i+1] && Buf_0[i]>Buf_1[i])
                 {
                  ...
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_SELECTABLE,false);
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_BACK,true);
                  if(alarmShow){Alert ( "The Best pro : BUY  "+Time[i]+"  "+Symbol()+"  Period Time "+timeFrame() );}
                  if(alarmShowNoti){SendNotification("The Best pro : BUY  "+Time[i]+"  "+Symbol()+"  Period Time  "+timeFrame());}
                 }
               if(Buf_0[i+1]>Buf_1[i+1] && Buf_0[i]<Buf_1[i])
                 {
                  ...
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_SELECTABLE,false);
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_BACK,true);
                  if(alarmShow){Alert ("The Best pro : SELL  "+Time[i]+"  "+Symbol()+"  Period Time "+timeFrame());}
                  if(alarmShowNoti){SendNotification("The Best pro : SELL  "+Time[i]+"  "+Symbol()+"  Period Time "+timeFrame());}
                 }
             }
       }
       ChartRedraw();
...
void creatModel(int xe,int ye){
   ...
   objB.creatObject(ChartID(),"backMenu1",OBJ_RECTANGLE_LABEL,5+xe,100+ye,154,200,"",clrYellowGreen,0,C'57,66,99',10,1,"",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"backMenu2",OBJ_RECTANGLE_LABEL,7+xe,102+ye,150,196,"",clrYellowGreen,0,C'57,66,99',10,1,"",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"text1",OBJ_LABEL,20+xe,170+53+ye,20,20,"---------------------",0,0,clrRed,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"buttonHiddenChart",OBJ_BUTTON,28+xe,155+ye,100,30,"Show Candles",C'20,167,172',C'20,167,172',clrWhite,10,0,"Tahoma Bold",
   C'145,151,176',clrWhite,clrYellow,C'89,93,138',clrWhite,clrRed,true,true);

   objB.creatObject(ChartID(),"text3",OBJ_LABEL,8+xe,140+ye,20,20,"___",0,0,clrRed,17,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"text4",OBJ_LABEL,60+xe,140+ye,20,20,"___",0,0,clrYellow,17,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"text5",OBJ_LABEL,110+xe,140+ye,20,20,"___",0,0,clrGreen,17,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"text6",OBJ_LABEL,18+xe,105+ye,20,20,"Long",0,0,C'145,151,176',8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrRed,clrRed,clrRed,clrRed,true,true);
   
   objB.creatObject(ChartID(),"text7",OBJ_LABEL,70+xe,105+ye,20,20,"Mid",0,0,C'145,151,176',8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrYellow,clrYellow,clrYellow,true,true);
   
   objB.creatObject(ChartID(),"text8",OBJ_LABEL,120+xe,105+ye,20,20,"Short",0,0,C'145,151,176',8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrGreen,clrGreen,clrGreen,clrGreen,true,true);
   
   objB.creatObject(ChartID(),"buttonMoveMenu",OBJ_BUTTON,141+xe,280+ye,15,15,"*",clrYellowGreen,clrYellowGreen,clrWhite,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"edit1",OBJ_EDIT,35+xe,260+ye,40,20,"100",C'88,93,117',C'180,50,50',clrWhite,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"buttonRun",OBJ_BUTTON,80+xe,260+ye,40,20,"Run",C'20,167,172',C'20,167,172',clrWhite,7,0,"Tahoma",
   C'145,151,176',clrWhite,clrYellow,C'89,93,138',clrWhite,clrRed,true,true);
   
   objB.creatObject(ChartID(),"text9",OBJ_LABEL,65+xe,235+ye,30,20,"Show Bar",C'20,167,172',C'20,167,172',clrWhite,7,0,"Tahoma",
   ...
   objB.creatObject(ChartID(),"buttonHiddenMenu",OBJ_BUTTON,10+xe,280+ye,15,15,"=",clrYellowGreen,clrYellowGreen,clrWhite,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"alarmText",OBJ_LABEL,55+xe,205+ye,20,20,"Alarm on",0,0,clrWhite,8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrGold,clrGold,clrGold,clrGold,true,true);
   
   objB.creatObject(ChartID(),"alarmTextNotification",OBJ_LABEL,40+xe,185+ye,20,20,"Notification on",0,0,clrWhite,8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrGold,clrGold,clrGold,clrGold,true,true);
   
   LabelCreate(0,"labelBreak",0,200,y_distance-40,0,"Menu not work in Tester!");
}
...
void OnChartEvent(const int id,         // Event ID 
   ...
   if(firstShow==false && creatMM && !IsTesting())
     {
       firstShow=true;
       int edi1=objB.getTextInt("edit1");
       if(edi1<=0)
  ...
  if(id==0 && lparam==82 && dparam==1 && sparam==19 )
   ...
   //---   button1
   if(sparam=="buttonHiddenChart")
     {
        if(objB.getTextStr("buttonHiddenChart")=="Show Candles")
          {
            objB.setText("buttonHiddenChart","Show Trend");
            fraw=false;
          }else{
            objB.setText("buttonHiddenChart","Show Candles");
            fraw=true;
          }
        setScale();
   ...
   if(sparam=="text7")
     {
       ...
       objB.setColorClick("text7");
       objB.offColorClick("text6");
       objB.offColorClick("text8");
     }
   //---   short
   if(sparam=="text8")
     {
       ...
       objB.nameObjectClick="";
       objB.setColorClick("text8");
       objB.offColorClick("text6");
       objB.offColorClick("text7");
     }
   if(sparam=="buttonRun")
...
void emptyObject(int id){ 
    ...
    obj[id].fontSize=0;
    ObjectSetInteger(obj[id].chart_id,obj[id].object_name,OBJPROP_XSIZE,0);
    ObjectSetInteger(obj[id].chart_id,obj[id].object_name,OBJPROP_YSIZE,0);
    ObjectSetInteger(obj[id].chart_id,obj[id].object_name,OBJPROP_XDISTANCE,0);
    ObjectSetInteger(obj[id].chart_id,obj[id].object_name,OBJPROP_YDISTANCE,0);
    ObjectSetInteger(obj[id].chart_id,obj[id].object_name,OBJPROP_FONTSIZE,0);
    ObjectSetString(obj[id].chart_id,obj[id].object_name,OBJPROP_TEXT,"");
    
    obj[id].hidden=true;
    //ObjectDelete(0,obj[id].object_name);
}
