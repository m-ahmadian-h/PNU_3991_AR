...
int cntBack=300;
int History;
input int periodsInput=60;
input bool alarmShow=false;
input bool alarmShowNoti=false;
int periods=60;
int endBar=1;
...
//--- indicator buffers mapping
   ...
   SetIndexStyle (0,DRAW_LINE,STYLE_SOLID,2, clrBlue);// Line style
   SetIndexLabel(0,"powerLongs");

   SetIndexBuffer(1,Buf_1);         // Assigning an array to a buffer
   SetIndexStyle (1,DRAW_LINE,STYLE_SOLID,2,clrRed);// Line style
           ...
           if(lastBar!=Bars)
             {
                ...
                Buf_1_USD=powerBaseShortIMAUSD;       // Value of 0 buffer on i bar
                Buf_0[i]=powerBaseLongIMAEUR-powerBaseLongIMAUSD;        // Value of 0 buffer on i bar
                Buf_1[i]=powerBaseShortIMAEUR-powerBaseShortIMAUSD; 
                //if(Buf_0[i]>0){Buf_1[i]=Buf_0[i];Buf_0[i]=NULL;}
                ///-----
               if(Buf_0[i+1]<Buf_1[i+1] && Buf_0[i]>Buf_1[i] )
                 ...
                 /*
                  ...
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_BACK,true);
                  */
                  if(alarmShow){Alert ( "Professinal trending pro : BUY  "+Time[i]+"  "+Symbol()+" "+timeFrame() );}
                  if(alarmShowNoti){SendNotification("Professinal trending pro : BUY  "+Time[i]+"  "+Symbol()+" "+timeFrame());}
                 }
               if(Buf_0[i+1]>Buf_1[i+1] && Buf_0[i]<Buf_1[i])
                 {
                  /*
                  ...
                  ObjectSetInteger(ChartID(),TimeToString(Time[i]),OBJPROP_BACK,true);
                  */
                  if(alarmShow){Alert ("Professinal trending pro : SELL  "+Time[i]+"  "+Symbol()+" "+timeFrame());}
                  if(alarmShowNoti){SendNotification("Trofessinal trending pro : SELL  "+Time[i]+"  "+Symbol()+" "+timeFrame());}
                 }
...
void creatModel(int xe,int ye){
   ...
   objB.creatObject(ChartID(),"backMenu1",OBJ_RECTANGLE_LABEL,5+xe,100+ye,154,124,"",C'89,93,138',0,clrWhiteSmoke,10,1,"",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"backMenu2",OBJ_RECTANGLE_LABEL,7+xe,102+ye,150,120,"",clrDarkBlue,0,clrWhiteSmoke,10,1,"",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"text1",OBJ_LABEL,20+xe,80+53+ye,20,20,"---------------------",0,0,clrRed,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,clrRed,clrRed,clrRed);
   
   objB.creatObject(ChartID(),"buttonMoveMenu",OBJ_BUTTON,140+xe,205+ye,15,15,"*",clrDarkBlue,clrDarkBlue,C'145,151,176',10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"edit1",OBJ_EDIT,65+xe,200+ye,40,20,periods,clrBisque,C'180,50,50',clrBlack,10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"buttonRun",OBJ_BUTTON,65+xe,170+ye,40,20,"Run",clrRed,C'20,167,172',clrWhiteSmoke,7,0,"Tahoma",
   C'145,151,176',clrWhite,clrYellow,C'89,93,138',clrWhite,clrRed,true,true);
   ...
   objB.creatObject(ChartID(),"buttonHiddenMenu",OBJ_BUTTON,10+xe,205+ye,15,15,"=",clrDarkBlue,clrDarkBlue,C'145,151,176',10,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrYellow,C'57,66,99',C'57,66,99',clrRed,true,true);
   
   objB.creatObject(ChartID(),"alarmText",OBJ_LABEL,55+xe,100+ye,20,20,"Alarm on",0,0,C'145,151,176',8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrGold,clrGold,clrGold,clrGold,true,true);
   
   objB.creatObject(ChartID(),"alarmTextNotification",OBJ_LABEL,40+xe,120+ye,20,20,"Notification on",0,0,C'145,151,176',8,0,"Tahoma Bold",
   C'57,66,99',C'57,66,99',clrGold,clrGold,clrGold,clrGold,true,true);
}
...
void OnChartEvent(const int id,         // Event ID 
  ...
  if(id==0 && lparam==82 && dparam==1 && sparam==19 )
   ...
   if(sparam=="buttonRun" || (firstShow==false && creatMM))
     {
       firstShow=true;
